...
def summarize_text(text: str, provider: str = "anthropic", model: Optional[str] = None) -> str:
    """
    ...
    if provider.lower() == "anthropic":
        ...
        try:
            ...
            if "deepseek" in model.lower():
                ...
                summary = re.sub(r'<\s*think\s*>.*?<\s*/\s*think\s*>', '', summary, flags=re.DOTALL).strip()
        except Exception as e:
...
def review_summary(summary: str, provider: str = "groq", model: Optional[str] = None) -> str:
    """
    ...
    if provider.lower() == "anthropic":
        ...
        try:
            ...
            response = client.messages.create(
                model=model,
                max_tokens=4000,
                system=("You are an expert at proofreading and improving summaries. You are given a summary of a transcript "
        ...
        try:
            ...
            response = groq_client.chat.completions.create(
                model=model,
                messages=[
                    {"role": "system", "content": "You are an expert at proofreading and improving summaries. You are given a summary of a transcript and you are tasked with improving it with the goal of increasing clarity, completeness, and accuracy."},
                    {"role": "user", "content": prompt},
                ]
            )
...
def main():
    try:
        ...
        try:
            ...
            print("\nGenerating summary...")
            summary = "1. " + summarize_text(cleaned_transcript, provider=provider_summary, model=model_summary)
            
            review_choice = input("\nWould you like for the summary to be reviewed? (Y/n): ").strip().lower()
            if review_choice in ["n", "no"]:
                reviewed_summary = summary
            else:
                print("\nSelect configuration for summary review:")
                provider_review, model_review = select_provider_and_model("summary review")
                print("\nReviewing summary...")
                reviewed_summary = "1. " + review_summary(summary, provider=provider_review, model=model_review)
